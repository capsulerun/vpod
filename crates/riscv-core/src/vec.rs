...
fn vreg_write_elem(
    vregs: &mut [[u8; VLEN_BYTES]; VREG_COUNT],
    reg: usize,
    elem: u64,
    sew_b: u64,
    value: u64,
...
fn effective_satp<B: SystemBus>(ctx: &ExecContext<B>) -> u64 {
    if *ctx.priv_mode == crate::csr::PrivMode::M {
        0
    } else {
        ctx.csr.satp
    }
...
fn exec_opcfg<B: SystemBus>(ctx: &mut ExecContext<B>, instruction_encoding: u32, pc: u64) -> StepResult {
    ...
    let bit30 = (instruction_encoding >> 30) & 1;

    let (avl, vtype_val) = if bit31 == 0 {
        let zimm = ((instruction_encoding >> 20) & 0x7FF) as u64;
        let avl = if rs1 == 0 && rd != 0 {
            u64::MAX
        } else if rs1 == 0 {
            ctx.csr.vl
        } else {
        ...
        let rs2 = ((instruction_encoding >> 20) & 0x1F) as usize;
        let avl = if rs1 == 0 && rd != 0 {
            u64::MAX
        } else if rs1 == 0 {
            ctx.csr.vl
        ...
        let zimm = ((instruction_encoding >> 20) & 0x3FF) as u64;
        (rs1 as u64, zimm)
    };
...
fn exec_vload<B: SystemBus>(ctx: &mut ExecContext<B>, instruction_encoding: u32, pc: u64) -> StepResult {
    ...
    let vd = ((instruction_encoding >> 7) & 0x1F) as usize;
    let rs1 = ((instruction_encoding >> 15) & 0x1F) as usize;
    let rs2 = ((instruction_encoding >> 20) & 0x1F) as usize;
    let vm = (instruction_encoding >> 25) & 1; // 1 = unmasked
    let mop = (instruction_encoding >> 26) & 0x3; // 0=unit 2=strided
    let nf = (instruction_encoding >> 29) & 0x7; // segment count-1; we only support nf=0
    ...
    let width_enc = (instruction_encoding >> 12) & 0x7; // 0=8,5=16,6=32,7=64
    let sew_b: u64 = match width_enc {
        0 => 1,
        5 => 2,
        6 => 4,
        7 => 8,
        _ => return StepResult::Trap(TrapCause::IllegalInstruction(instruction_encoding)),
    };

    let base = ctx.regs.read(rs1);
    let stride: i64 = if mop == 2 {
        ctx.regs.read(rs2) as i64
    } else {
        sew_b as i64
    };
    let vl = ctx.csr.vl;
    ...
    for i in 0..vl {
        ...
        let pa = match vec_translate_load(ctx, va) {
            Ok(p) => p,
            Err(fault) => {
                take_exception(ctx, fault.mcause(), fault.tval());
                return StepResult::Ok;
            }
        };
        let value = match sew_b {
        ...
        vreg_write_elem(ctx.vregs, vd, i, sew_b, value);
    }
...
fn exec_vstore<B: SystemBus>(ctx: &mut ExecContext<B>, instruction_encoding: u32, pc: u64) -> StepResult {
    ...
    let vs3 = ((instruction_encoding >> 7) & 0x1F) as usize;
    let rs1 = ((instruction_encoding >> 15) & 0x1F) as usize;
    let rs2 = ((instruction_encoding >> 20) & 0x1F) as usize;
    let vm = (instruction_encoding >> 25) & 1;
    let mop = (instruction_encoding >> 26) & 0x3;
    let nf = (instruction_encoding >> 29) & 0x7;
    ...
    let width_enc = (instruction_encoding >> 12) & 0x7;
    let sew_b: u64 = match width_enc {
        0 => 1,
        5 => 2,
        6 => 4,
    ...
    for i in 0..vl {
        ...
        let value = vreg_read_elem(ctx.vregs, vs3, i, sew_b);
        let va = base.wrapping_add((stride * i as i64) as u64);
        ...
        match sew_b {
            1 => ctx.bus.write_byte(pa, value as u8),
            2 => ctx.bus.write_halfword(pa, value as u16),
            4 => ctx.bus.write_word(pa, value as u32),
            8 => ctx.bus.write_doubleword(pa, value),
            _ => unreachable!(),
        }
    }
...
fn src_scalar(
    ...
    if vv {
        let vs1 = ((instruction_encoding >> 15) & 0x1F) as usize;
        vreg_read_elem(vregs, vs1, elem, sew_b)
    } else if vx {
        let rs1 = ((instruction_encoding >> 15) & 0x1F) as usize;
        regs.read(rs1)
    } else {
        let imm5 = ((instruction_encoding >> 15) & 0x1F) as i64;
        ((imm5 << 59) >> 59) as u64
    }
}
...
fn exec_opivv_opivx_opivi<B: SystemBus>(
    ...
    let vd = ((instruction_encoding >> 7) & 0x1F) as usize;
    let vs2 = ((instruction_encoding >> 20) & 0x1F) as usize;
    let vm = (instruction_encoding >> 25) & 1;
    ...
    for i in 0..vl {
        ...
        let b = src_scalar(instruction_encoding, vv, vx, ctx.regs, ctx.vregs, i, sew_b);
...
fn exec_opmvv_opmvx<B: SystemBus>(
    ...
    // vmv.x.s
    if funct6 == 0x10 && vv && ((instruction_encoding >> 15) & 0x1F) == 0 {
        let value = vreg_read_elem(ctx.vregs, vs2, 0, sew_b);

        let xrd = ((instruction_encoding >> 7) & 0x1F) as usize;
        if xrd != 0 {
            ctx.regs.write(xrd, sign_extend_sew(value, sew_bits) as u64);
        }
    ...
    if funct6 == 0x10 && !vv {
        if vl >= 1 {
            let rs1 = ((instruction_encoding >> 15) & 0x1F) as usize;
            let value = ctx.regs.read(rs1) & mask_bits;
            vreg_write_elem(ctx.vregs, vd, 0, sew_b, value);
            mark_vs_dirty(ctx);
        }
        ctx.csr.vstart = 0;
        ctx.regs.pc = pc.wrapping_add(4);
    ...
    for i in 0..vl {
        ...
        let result: u64 = match funct6 {
            ...
            0x27 => mulh(
                sign_extend_sew(a, sew_bits),
                sign_extend_sew(b, sew_bits),
                sew_bits,
            ),
            _ => return StepResult::Trap(TrapCause::IllegalInstruction(instruction_encoding)),
        };
...
fn set_mask_bit(vregs: &mut [[u8; VLEN_BYTES]; VREG_COUNT], reg: usize, elem: u64, value: bool) {
    let byte = (elem / 8) as usize;
    let bit = (elem % 8) as u32;
...
fn sign_extend_sew(value: u64, sew_bits: u64) -> i64 {
