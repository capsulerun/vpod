...
impl TlsContext {
    pub fn new() -> Result<Self, String> {
        ...
        provider
            .cipher_suites
            .sort_by_key(|cs| u8::from(!format!("{:?}", cs.suite()).contains("CHACHA20")));
...
impl TlsProxy {
    ...
    pub(crate) fn with_timing(
        ...
        let mut server =
            ServerConnection::new(ctx.server_config.clone()).map_err(|e| e.to_string())?;
    ...
    fn pump(&mut self) {
        ...
        if let Err(e) = self.server.process_new_packets() {
            log::warn!("tls_proxy: guest-side handshake failed: {e}");
            self.failed = true;
            return;
        }
